...
with lib; let
  ...
  postgresqlBackupService = db: dumpCmd: let
    ...
    compressCmd = getAttr cfg.compression {
      "none" = "cat";
      "gzip" = "${pkgs.gzip}/bin/gzip -c -${toString cfg.compressionLevel} ${optionalString cfg.compressionRsyncable "--rsyncable"}";
      "zstd" = "${pkgs.zstd}/bin/zstd -c -${toString cfg.compressionLevel} ${optionalString cfg.compressionRsyncable "--rsyncable"}";
    };
...
in {
  ...
  options = {
    services.postgresqlBackup = {
      ...
      compressionRsyncable = mkEnableOption "rsyncable option of compression program";
    };
  };
  ...
  config = mkMerge [
    {
      assertions = [
        ...
        {
          assertion =
            (cfg.compressionRsyncable -> cfg.compression == "gzip")
            || (cfg.compressionRsyncable -> cfg.compression == "zstd");
          message = "config.services.postgresqlBackup.compressionRsyncable is only supported for gzip and zstd modes";
        }
      ];
    }
    (mkIf cfg.enable {
    ...
    (mkIf (cfg.enable && !cfg.backupAll) {
      systemd.services = listToAttrs (map (db: let
